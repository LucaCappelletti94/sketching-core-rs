...
| 4 | 2026-07-01 | v0.5.0 (e5872a0) | e5872a0 | e5872a0 | e5872a0 | `SplitMix64` (stateful wrapper), `DefaultHasher`, and `DefaultBuildHasher` moved from `setsketch` to `sketching-core::random` and `sketching-core::hasher`. `setsketch::hash` and `setsketch::utils` become thin re-export modules. `hyperloglog-rs` and `hyperlogloglog-rs` pin bumps only (they keep `twox_hash::XxHash64` as the default hasher and can opt into the new default separately). |
| 5 | pending | v0.6.0 | | | | |
